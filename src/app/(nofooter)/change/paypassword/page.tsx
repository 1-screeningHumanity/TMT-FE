  ...
  async function handleSetPayPassword(){
    if(password === checkPassword && password.length === 4 && checkPassword.length === 4){
      ...
      if(response.code == 200){
        alert("비밀번호가 변경되었습니다.");
        location.href="/change/paypassword/complete";
      }
  ...
  return(
    <>
      ...
      { step ?
        ...
        (<div className="w-fit mx-auto absolute bottom-6 left-0 right-0">
          <input type="button" className="rounded-lg w-40 h-14 font-bold text-center mx-2 text-white bg-black" value={"이전으로"} onClick={() => history.go(-1)}/>

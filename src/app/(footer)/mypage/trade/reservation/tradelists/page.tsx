...
export default async function reservationTradeLists() {
  ...
  console.log(reservationTradeLists)
  return (
    <section>
      ...
      <div className="overflow-x-scroll mb-20">
        {reservationTradeLists.length == 0 ? (
          ...
          reservationTradeLists.map((trade: ReservationTradeListsType) => (
            <Link href={`/stock/${trade.stockCode}`}>
              <div
                key={trade.id}
                className="flex gap-2 text-md text-center mt-2 py-4 font-[Pretendard-Regular] pl-4 leading-8"
              >
